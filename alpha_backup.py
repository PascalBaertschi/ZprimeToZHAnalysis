...
parser.add_option("-y", "--year", action="store", default='2017', dest="year")
...
massPoints = [x for x in range(800, 6000+1, 100)] #if not HVTMODEL else genPoints
...
# inclusive top SF
topSF = {
    'nnbb'  : [1.339, 0.113, 0.042],
    'embb'  : [1.405, 0.102, 0.004],
    'eebb'  : [1.405, 0.102, 0.083],
    'mmbb'  : [1.405, 0.102, 0.104],
    'nn0b'  : [1.094, 0.046, 0.044],
    'em0b'  : [1.151, 0.024, 0.007],
    'ee0b'  : [1.151, 0.024, 0.083],
    'mm0b'  : [1.151, 0.024, 0.103],
    'nnbbVBF'  : [1.339, 0.113, 0.042],
    'embbVBF'  : [1.405, 0.102, 0.004],
    'eebbVBF'  : [1.405, 0.102, 0.083],
    'mmbbVBF'  : [1.405, 0.102, 0.104],
    'nn0bVBF'  : [1.094, 0.046, 0.044],
    'em0bVBF'  : [1.151, 0.024, 0.007],
    'ee0bVBF'  : [1.151, 0.024, 0.083],
    'mm0bVBF'  : [1.151, 0.024, 0.103]}


vvSF = [1.000, 0.229]

functions = {
    "nnbb" : {"Vjet" : "POL5", "VjetAlt" : "EXPGAUS", "Top" : "GAUS2", "VV" : "ERFEXPGAUS",  "Shape" : "EXPN", "ShapeAlt" : "EXPTAIL"},
    "eebb" : {"Vjet" : "POL5", "VjetAlt" : "EXPGAUS", "Top" : "GAUS",  "VV" : "ERFEXPGAUS",  "Shape" : "EXPN", "ShapeAlt" : "EXPTAIL"},
    "mmbb" : {"Vjet" : "POL5", "VjetAlt" : "EXPGAUS", "Top" : "GAUS2", "VV" : "ERFEXPGAUS",  "Shape" : "EXPN", "ShapeAlt" : "EXPTAIL"},
    "nn0b" : {"Vjet" : "POL5", "VjetAlt" : "EXPGAUS", "Top" : "GAUS3", "VV" : "ERFEXPGAUS",  "Shape" : "POW2", "ShapeAlt" : "EXPTAIL"},
    "ee0b" : {"Vjet" : "POL5", "VjetAlt" : "EXPGAUS", "Top" : "GAUS", "VV" : "ERFEXPGAUS",  "Shape" : "POW2", "ShapeAlt" : "EXPTAIL"},
    "mm0b" : {"Vjet" : "POL5", "VjetAlt" : "EXPGAUS", "Top" : "GAUS2", "VV" : "ERFEXPGAUS", "Shape" : "POW2", "ShapeAlt" : "EXPTAIL"},
    "nnbbVBF" : {"Vjet" : "POL2", "VjetAlt" : "EXP", "Top" : "GAUS2", "VV" : "GAUS2",  "Shape" : "EXPN", "ShapeAlt" : "EXPTAIL"},
    "eebbVBF" : {"Vjet" : "EXP", "VjetAlt" : "POW", "Top" : "GAUS",  "VV" : "ERFEXP",  "Shape" : "EXPN", "ShapeAlt" : "EXPTAIL"},
    "mmbbVBF" : {"Vjet" : "EXP", "VjetAlt" : "POW", "Top" : "GAUS2", "VV" : "ERFEXPGAUS",  "Shape" : "EXPN", "ShapeAlt" : "EXPTAIL"},
    "nn0bVBF" : {"Vjet" : "POL5", "VjetAlt" : "EXPGAUS", "Top" : "GAUS3", "VV" : "ERFEXPGAUS",  "Shape" : "POW2", "ShapeAlt" : "EXPTAIL"},
    "ee0bVBF" : {"Vjet" : "POL5", "VjetAlt" : "EXPGAUS", "Top" : "ERFEXP", "VV" : "ERFEXPGAUS",  "Shape" : "POW2", "ShapeAlt" : "EXPTAIL"},
    "mm0bVBF" : {"Vjet" : "POL5", "VjetAlt" : "EXPGAUS", "Top" : "GAUS2", "VV" : "ERFEXPGAUS", "Shape" : "POW2", "ShapeAlt" : "EXPTAIL"},
    }
...
def alpha(channel):
    ...
    if '0b' in channel:
        nBtag = 0
    isVBF = True if 'VBF' in channel else False
    ...
    if nBtag==0: X_mass.setMin(XTBINMIN); X_mass.setMax(XTBINMAX)
    ...
    variables.add(RooArgSet(isVBF, Mu1_relIso, Mu2_relIso))
...
#    if nLept==0: Xbins /= 2
    ...
    widthTop  = RooRealVar("widthTop",  "width of the erf",  50.0,    1., 130.)#100 130
    ...
    if 'VBF' in channel:
        ...
        if nElec==2 and nBtag==2:
            a1Vjet.setConstant(True)
        if nLept==0 or nLept==2 or nBtag==2: 
    ...
    if not EXTRAPOLATE:
        if nBtag == 2 and not isVBF:
            if nLept==0:
                par["denVVSR"].setMin(-2500)
                par["denVVSR"].setVal(0)
                par["denVVSR"].setMax(1200)
                par["numVVSR"].setMin(-6.e-3)
                par["numVVSR"].setVal(-4.5e-3)
                par["numVVSR"].setMax(-4.e-3)
                par["denVVSB"].setMin(-4000)
                par["denVVSB"].setVal(0)
                par["denVVSB"].setMax(4000)
                ...
                par["numTopSB"].setMin(-12.e-3)
                par["numTopSB"].setVal(-8.e-3)
                par["numTopSB"].setMax(-7.e-3)
                par["denTopSR"].setMin(2600)
                par["denTopSR"].setVal(3300)
                par["denTopSR"].setMax(6000)
                par["numTopSR"].setMin(-5.e-3)
                par["numTopSR"].setVal(-4.e-3)
                par["numTopSR"].setMax(-3.e-3)
                par["c0TopSR"].setMin(-100)
                par["c0TopSR"].setVal(40)
                par["c0TopSR"].setMax(100)
                par["c0VVSR"].setMin(-1000)
                par["c0VVSR"].setMax(1000)
            if nElec==2:
                par["denMainSB"].setMin(0.)
                par["denMainSB"].setMax(1.e4)
                par["denTopSB"].setMin(-4000)#-3000
                par["denTopSB"].setVal(-600)#-600 
                par["denTopSB"].setMax(2000)#2000  
                par["denTopSR"].setMin(500)
                par["denTopSR"].setVal(1800)
                par["denTopSR"].setMax(5000)
                par["denVVSB"].setMin(-6000)
                par["denVVSB"].setVal(-2000)
                par["denVVSB"].setMax(-1000)
                par["denVVSR"].setMin(3700)
                par["denVVSR"].setVal(4000)
                ...
                par["denVjetSR"].setVal(5100)
                par["denVjetSR"].setMax(7500)
                par["numVjetSR"].setMin(-5.e-3)
                par["numVjetSR"].setVal(-3.5e-3)
                par["numVjetSR"].setMax(-3.e-3)
          
            if nMuon==2:
                par["numVjetSB"].setMin(-3.e-3)
                par["numVjetSB"].setVal(-2.4e-3)
                par["numVjetSB"].setMax(-2.e-3)
                par["numVjetSR"].setMin(-5.e-3)
                par["numVjetSR"].setVal(-3.8e-3)
                par["numVjetSR"].setMax(-3.e-3)
                par["denTopSB"].setMin(300)
                par["denTopSB"].setVal(1500)
                par["denTopSB"].setMax(3500)
                par["denVVSB"].setMin(-3000)
                par["denVVSB"].setVal(-1500)
                par["denVVSB"].setMax(-500)
                par["denVVSR"].setMin(-1800)
                par["denVVSR"].setVal(-500)
                par["denVVSR"].setMax(1000)
                par["c0TopSR"].setMin(-400)
                par["c0TopSR"].setVal(40)
                par["c0TopSR"].setMax(400)
                par["c0MainSB"].setMin(100.)
                par["c0MainSB"].setMax(1.e3)
        
        elif nBtag==0 and not isVBF:
            if nLept==0:
                par["c0MainSB"].setVal(80)
                par["p0VVSB"].setMin(18)
                par["p0VVSB"].setVal(30)
                par["p0VVSB"].setMax(35)
                par["p1VVSB"].setMin(1)
                par["p1VVSB"].setVal(2)
                par["p1VVSB"].setMax(5)
                par["p0VVSR"].setMin(10)
                par["p0VVSR"].setVal(30)
                par["p0VVSR"].setMax(90)
                par["c1TopSR"].setMin(1.e-2)
                par["c1TopSR"].setVal(2.4e-2)
                par["c1TopSR"].setMax(4.e-2)
            if nElec==2:
                par["p0VVSR"].setMin(5)
                par["p0VVSR"].setVal(40)
                par["p0VVSR"].setMax(130)
                par["p0TopSB"].setMin(5)
                par["p0TopSB"].setVal(60)
                par["p0TopSB"].setMax(230)
                par["p1TopSB"].setMin(0)
                par["p1TopSB"].setVal(10)
                par["p1TopSB"].setMax(50)
                par["p0TopSR"].setMin(40)
                par["p0TopSR"].setVal(50)
                par["p0TopSR"].setMax(60)
                par["p1TopSR"].setMin(1)
                par["p1TopSR"].setVal(3)
                par["p1TopSR"].setMax(5)
                par["p0VjetSR"].setMin(10)
                par["p0VjetSR"].setVal(30)
                par["p0VjetSR"].setMax(80)
                par["c0VVSR"].setMin(-1000)
                par["c0VVSR"].setVal(150)
                par["c0VVSR"].setMax(1000)
            if nMuon==2:
                par["c0TopSR"].setMin(-1000)
                par["c0TopSR"].setVal(150)
                par["c0TopSR"].setMax(1000)
                par["p0TopSB"].setMin(40)
                par["p0TopSB"].setVal(70)
                par["p0TopSB"].setMax(270)
                par["p0VVSR"].setMin(0)
                par["p0VVSR"].setVal(30)
                par["p0VVSR"].setMax(40)
                par["p1VVSR"].setMin(0)
                par["p1VVSR"].setVal(3)
                par["p1VVSR"].setMax(5)
                par["p0VVSB"].setMin(0)
                par["p0VVSB"].setVal(30)
                par["p0VVSB"].setMax(40)
                par["p1VVSB"].setMin(0)
                par["p1VVSB"].setVal(2)
                par["p1VVSB"].setMax(4)
        elif nBtag == 2 and isVBF:
            if nLept==0:
                par["denVVSR"].setMin(800)
                par["denVVSR"].setVal(1500)
                par["denVVSR"].setMax(2200)
                #par["numVVSR"].setMin(-6.e-3)
                #par["numVVSR"].setVal(-4.5e-3)
                #par["numVVSR"].setMax(-4.e-3)
                par["denVVSB"].setMin(500)#0
                par["denVVSB"].setVal(2500)
                par["denVVSB"].setMax(4000)
                par["numVVSB"].setMin(-4.e-3)
                par["numVVSB"].setVal(-3.e-3)
                par["numVVSB"].setMax(-1.e-3)
                par["denTopSB"].setMin(4000)
                par["denTopSB"].setVal(5000)
                par["denTopSB"].setMax(9000)#8000
                par["numTopSB"].setMin(-3.e-3)
                par["numTopSB"].setVal(-2.e-3)
                par["numTopSB"].setMax(-1.e-3)
                par["denTopSR"].setMin(7000)#7000
                par["denTopSR"].setVal(8800)
                par["denTopSR"].setMax(9700)#9700
                par["numTopSR"].setMin(-6.e-3)
                par["numTopSR"].setVal(-4.e-3)
                par["numTopSR"].setMax(-2.e-3)
                par["denVjetSR"].setMin(0)
                par["denVjetSR"].setVal(1500)
                par["denVjetSR"].setMax(3500)#3500
                par["numVjetSR"].setMin(-6.e-3)
                par["numVjetSR"].setVal(-5.e-3)
                par["numVjetSR"].setMax(-4.e-3)
                par["denVjetSB"].setMin(1000)#1000
                par["denVjetSB"].setVal(2700)
                par["denVjetSB"].setMax(5000)#4000
                #par["denVjetSR"].setMin(0)
                #par["denVjetSR"].setVal(1500)
                #par["denVjetSR"].setMax(5000)#3500
            if nElec==2:
                par["denVVSB"].setMin(2300)
                par["denVVSB"].setVal(3000)
                par["denVVSB"].setMax(3700)
                par["numVVSB"].setMin(-4.e-3)
                par["numVVSB"].setVal(-3.e-3)
                par["numVVSB"].setMax(-1.e-3)
                par["denVjetSB"].setMin(-500)
                par["denVjetSB"].setVal(100)
                par["denVjetSB"].setMax(3500)#1500
                par["denVjetSR"].setMin(200)
                par["denVjetSR"].setVal(1200)
                par["denVjetSR"].setMax(3200)#4500
                par["numVjetSR"].setMin(-5.e-3)
                par["numVjetSR"].setVal(-4.e-3)
                par["numVjetSR"].setMax(-3.e-3)
                par["numVjetSB"].setMin(-5.e-3)
                par["numVjetSB"].setVal(-4.e-3)
                par["numVjetSB"].setMax(-3.e-3)
            if nMuon==2:
                par["numVjetSB"].setMin(-5.e-3)
                par["numVjetSB"].setVal(-3.5e-3)
                par["numVjetSB"].setMax(-2.e-3)
                par["numVjetSR"].setMin(-4.e-3)#-5.e-3
                par["numVjetSR"].setVal(-3.5e-3)
                par["numVjetSR"].setMax(-2.e-3)
                par["denTopSB"].setMin(1300)
                par["denTopSB"].setVal(2200)
                par["denTopSB"].setMax(3300)
                par["denTopSR"].setMin(3400)
                par["denTopSR"].setVal(4000)
                par["denTopSR"].setMax(6000)
                par["numTopSR"].setMin(-5.e-3)
                par["numTopSR"].setVal(-4.e-3)
                par["numTopSR"].setMax(-2.e-3)
                par["denVVSB"].setMin(1600)
                par["denVVSB"].setVal(2600)
                par["denVVSB"].setMax(3200)
                par["denVjetSR"].setMin(-600)#
                par["denVjetSR"].setVal(2100)
                par["denVjetSR"].setMax(3000)#3000
                par["denVjetSB"].setMin(-500)#200
                par["denVjetSB"].setVal(1200)
                par["denVjetSB"].setMax(3000)#3000
        elif nBtag==0 and isVBF:
            if nLept==0:
                par["p0VVSB"].setMin(45)#40
                par["p0VVSB"].setVal(60)#60
                par["p0VVSB"].setMax(140)#95
                #par["p1VVSB"].setMin(1)
                #par["p1VVSB"].setVal(2)
                #par["p1VVSB"].setMax(5)
                par["p0VVSR"].setMin(5)
                par["p0VVSR"].setVal(10)
                par["p0VVSR"].setMax(65)#20
                par["p0TopSR"].setMin(0)
                par["p0TopSR"].setVal(10)
                par["p0TopSR"].setMax(60)#40
            if nElec==2:
                par["p0TopSB"].setMin(60)
                par["p0TopSB"].setVal(70)
                par["p0TopSB"].setMax(130)
                par["p0TopSR"].setMin(0)
                par["p0TopSR"].setVal(5)
                par["p0TopSR"].setMax(75)#15
                """
                par["p0VVSR"].setMin(5)
                par["p0VVSR"].setVal(40)
                par["p0VVSR"].setMax(130)
                ...
                """  
            if nMuon==2:
                par["p0VVSR"].setMin(15)
                par["p0VVSR"].setVal(40)
                par["p0VVSR"].setMax(70)#70
                par["p0VVSB"].setMin(15)
                par["p0VVSB"].setVal(40)
                par["p0VVSB"].setMax(120)#120
                """
                par["c0TopSR"].setMin(-1000)
                par["c0TopSR"].setVal(150)
                par["c0TopSR"].setMax(1000)
    ...
    syst_trig = {'nnbb' : 0.030, 'eebb' : 0.060, 'mmbb' : 0.020, 'nn0b' : 0.030, 'ee0b' : 0.058, 'mm0b' : 0.017,'nnbbVBF' : 0.030, 'eebbVBF' : 0.060, 'mmbbVBF' : 0.020, 'nn0bVBF' : 0.030, 'ee0bVBF' : 0.058, 'mm0bVBF' : 0.017}
    syst_elec = {'nnbb' : 0.010, 'eebb' : 0.083, 'mmbb' : 0.000, 'nn0b' : 0.010, 'ee0b' : 0.083, 'mm0b' : 0.000,'nnbbVBF' : 0.010, 'eebbVBF' : 0.083, 'mmbbVBF' : 0.000, 'nn0bVBF' : 0.010, 'ee0bVBF' : 0.083, 'mm0bVBF' : 0.000}
    syst_muon = {'nnbb' : 0.010, 'eebb' : 0.000, 'mmbb' : 0.005, 'nn0b' : 0.010, 'ee0b' : 0.000, 'mm0b' : 0.005,'nnbbVBF' : 0.010, 'eebbVBF' : 0.000, 'mmbbVBF' : 0.005, 'nn0bVBF' : 0.010, 'ee0bVBF' : 0.000, 'mm0bVBF' : 0.005}
    #syst_trig = {'nnbb' : 0.030, 'eebb' : 0.029, 'mmbb' : 0.035, 'nn0b' : 0.030, 'ee0b' : 0.029, 'mm0b' : 0.035}
    #syst_elec = {'nnbb' : 0.010, 'eebb' : 0.036, 'mmbb' : 0.000, 'nn0b' : 0.010, 'ee0b' : 0.036, 'mm0b' : 0.000}
    #syst_muon = {'nnbb' : 0.010, 'eebb' : 0.000, 'mmbb' : 0.059, 'nn0b' : 0.010, 'ee0b' : 0.000, 'mm0b' : 0.059}
    ...
    if not nBtag==0:
        syst[PREFIX+"eff_b"] = { 800 : [1.028, 0.973],  1000 : [1.031, 0.970],  1100 : [1.033, 0.967],  1200 : [1.036, 0.965],  1300 : [1.039, 0.962],  1400 : [1.042, 0.959],  1500 : [1.044, 0.956],  1600 : [1.047, 0.954],  1700 : [1.049, 0.952],  1800 : [1.051, 0.950],  2000 : [1.054, 0.947],  2500 : [1.061, 0.941],  3000 : [1.065, 0.937],  3500 : [1.068, 0.934],  4000 : [1.070, 0.932],  4500 : [1.072, 0.931], 5000 : [1.072, 0.931], 5500 : [1.072, 0.931], 6000 : [1.072, 0.931],}
    if nBtag==0:
        syst[PREFIX+"eff_b"] = { 800 : [1.027, 0.972],  1000 : [1.031, 0.968],  1100 : [1.035, 0.964],  1200 : [1.038, 0.960],  1300 : [1.041, 0.957],  1400 : [1.045, 0.954],  1500 : [1.046, 0.952],  1600 : [1.048, 0.950],  1700 : [1.048, 0.950],  1800 : [1.048, 0.949],  2000 : [1.048, 0.949],  2500 : [1.045, 0.952],  3000 : [1.037, 0.960],  3500 : [1.029, 0.967],  4000 : [1.023, 0.973],  4500 : [1.019, 0.978], 5000 : [1.019, 0.978], 5500 : [1.019, 0.978], 6000 : [1.019, 0.978],}
    ...
    for m in massPoints:
        if nBtag==0 and tau21==0.55:
            syst[PREFIX+"extr_V"] = 0.041*np.log(m/400)
        if nBtag==0 and tau21==0.35:
            syst[PREFIX+"extr_V"] = 0.085*np.log(m/400)
