...
def getChannel(channel):
    ...
    #
    if 'ZHslbbVBF' in channel: return "0l, 2l b tag categories, VBF"
    if 'ZHsl0bVBF' in channel: return "0l, 2l no btag categories, VBF"
    if 'ZHslVBF' in channel: return "0l, 2l categories, VBF"
    if 'ZHslbbcomb' in channel: return "0l, 2l b tag categories"
    if 'ZHsl0bcomb' in channel: return "0l, 2l no btag categories"
    if 'ZHslcomb' in channel: return "0l, 2l categories"
    if 'ZHslbb' in channel: return "0l, 2l b tag categories, no VBF"
    if 'ZHsl0b' in channel: return "0l, 2l no btag categories, no VBF"
    if 'ZHsl' in channel: return "0l, 2l categories, no VBF"
    if 'sum' in channel: return "Total"
    # leptons
    if 'nn' in channel or '0l' in channel: text += "0l"
    elif 'ee' in channel or '2e' in channel: text += "2e"
    elif 'e' in channel or '1e' in channel: text += "1e1#mu"
    elif 'mm' in channel or '2m' in channel: text += "2#mu"
    elif 'm' in channel or '1m' in channel: text += "1e1#mu"
    ...
    # b-tag
    if '0b' in channel: text += ", no b tag"
    elif 'bb' in channel or '2b' in channel: text += ", b tag"
    elif 'sl' in channel: text += ", 0 + 2 b tag"
    # VBF
    if 'VBF' in channel: text += ", VBF"
    else: text += ", no VBF"
    # region
...
def drawAnalysis(s, center=False):
    ...
    if s.startswith('X'):
        if 'bb' in s:
            if "nn" in s: text = "X #rightarrow Zh #rightarrow #nu#nub#bar{b}"
            elif "ee" in s: text = "X #rightarrow Zh #rightarrow eeb#bar{b}"
            elif "mm" in s: text = "X #rightarrow Zh #rightarrow #mu#mub#bar{b}"
        elif 'inc' in s:
            if "nn" in s: text = "X #rightarrow Zh #rightarrow #nu#nuincl"
            elif "ee" in s: text = "X #rightarrow Zh #rightarrow eeincl"
            elif "mm" in s: text = "X #rightarrow Zh #rightarrow #mu#muincl"
        elif '0b' in s:
            if "nn" in s: text = "X #rightarrow Zh #rightarrow #nu#nu0b"
            elif "ee" in s: text = "X #rightarrow Zh #rightarrow ee0b"
            elif "mm" in s: text = "X #rightarrow Zh #rightarrow #mu#mu0b"    
